...
await micropip.install(wheels["blueprint"], deps=False)

example = importlib.import_module(f"spaday_blueprint.{example_name}")
...
async def call_endpoint(path, body_json):
    ...
    if path == "/api/deployments":
        response = await example.deploy(LocalRequest(body))
    elif match := re.fullmatch(r"/api/services/([^/]+)/restart", path):
        response = await example.restart(LocalRequest(path_params={"id": match.group(1)}))
    elif match := re.fullmatch(r"/api/incidents/([^/]+)/acknowledge", path):
        response = await example.acknowledge(LocalRequest(path_params={"id": match.group(1)}))
    else:
...
if hasattr(example, "stream_release"):
    asyncio.create_task(example.stream_release())
